...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('parallel', 'Spawn child grunt processes to run the given tasks in parallel', function(){
    var exec = require('child_process').exec;
    var done = this.async();
    var pendingTasks = this.data.length;

    this.data.forEach(function (task) {
      var task = util.parallelTask(task);
      exec('grunt ' + task.args.join(' '), function (err, stdout, stderr) {
        if (err) {
          grunt.log.error(err, stderr);
          process.exit(1); // lol
          return;
        }
        grunt.log.ok(task + ': ' + stdout);
        pendingTasks -= 1;
        if (pendingTasks === 0) {
          done();
        }
